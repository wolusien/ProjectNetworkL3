...
public class tools{
	...
	/*###############################################"###########################################################"
	 */
	public static  String mess_app(String id_app,String mess, Entity e){
		...
		if(id_app.equals("LETTRE##")){
			return "APPL "+remplissageId(idm)+" "+remplissageId(id_app)+" DEV "+remplissageIp2(ip())+" "+e.getCli().port+" "+alealettre();
		}
		return "APPL "+remplissageId(idm)+" "+remplissageId(id_app)+" "+mess;
	}
	
	public static String mess_app_rep_lettre(Entity e,String id_app,String lettre, String mess){
		String idm=genereIdm();
		return "APPL "+remplissageId(idm)+" "+remplissageId(id_app)+" REP "+remplissageIp2(ip())+" "+e.getCli().port+" "+lettre+" "+mess;
	}
	
	public static String mess_app_gagnant_lettre(String id_app, Entity e){
		String idm=genereIdm();
		return "APPL "+remplissageId(idm)+" "+remplissageId(id_app)+" GAG "+remplissageIp2(ip())+" "+e.getCli().port+" "+e.getId();
	}
	
	public static String alealettre(){
		String s="azertyuiopqsdfghjklmwxcvbn";
		int r=(int)(Math.random()*26);
		return Character.toString(s.charAt(r));
	}
...
####################################################################
	...
	public static boolean verif_mess_app_lettre_dev(String[] decomp){
		if(decomp.length!=7)return false;
		if(!(decomp[0].equals("APPL") 
				&& decomp[2].equals("LETTRE##")
				&& decomp[3].equals("DEV"))) return false;		
		return true;
	}
	
	public static boolean verif_mess_app_rep(String[] decomp){
		if(decomp.length!=8) return false;
		if(!(decomp[0].equals("APPL") 
				&& decomp[2].equals("LETTRE##") 
				&& decomp[3].equals("REP") 
				&& decomp[7].startsWith(decomp[6])))
			return false;
		return true;
	}
	
	public static boolean verif_mess_app_gag(String[] decomp){
		if(decomp.length!=7) return false;
		if(!(decomp[0].equals("APPL")
				&& decomp[2].equals("LETTRE##")
				&& decomp[3].equals("GAG")))
